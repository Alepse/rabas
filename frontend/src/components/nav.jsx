    ...
    <span>
      {parts.map((part, i) =>
        part.toLowerCase() === match.toLowerCase().trim() ? (
          <span key={i} className=" font-extrabold text-color2/80 rounded">
            {part.trim()}
          </span>
        ) : (
            ...
            {isLoggedIn ? (
              <Dropdown className='bg-light' placement="bottom-end">
                ...
                <DropdownMenu  >
                  ...
                  <DropdownItem key="logout" onClick={handleLogout}>
                    <div className="flex gap-1 items-center w-full text-red-500 text-left p-1"> <LuLogOut className="w-5 h-6" />Logout</div>
                  </DropdownItem>
                </DropdownMenu>
              </Dropdown>
            ) : (
