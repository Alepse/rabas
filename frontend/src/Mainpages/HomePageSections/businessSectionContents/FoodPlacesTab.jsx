...
const FoodPlacesTab = ({foodPlacesData, loading}) => {
  ...
  return (
    <div className='lg:container'>
      <FoodPlaceSwiper 
        title="Culinary Delights: Must-Try Food Spots" 
        link="/foodplaces" 
        foodPlaces={culinaryDelights} 
        loading={loading} 
        uniqueId="culinaryDelights"
        />
      <FoodPlaceSwiper 
        title="Taste-Tested: Most Liked Eateries" 
        link="/foodplaces" 
        foodPlaces={tasteTested} 
        loading={loading} 
        uniqueId="tasteTested"
      />
      <FoodPlaceSwiper 
...
const FoodPlaceSwiper = ({ title, link, isLast, foodPlaces, loading, uniqueId }) => {
                ...
                <div className="flex-grow flex flex-col justify-between mt-4 px-2">
                        ...
                        {foodPlace.rating ? (
                          <>
                            <span className="text-[12px]">
                              {parseFloat(foodPlace.rating).toFixed(1)}
                            </span>
                            <span className="text-yellow-500">
                              {'★'.repeat(Math.floor(foodPlace.rating))}
                              {'☆'.repeat(5 - Math.floor(foodPlace.rating))}
                            </span>
                          </>
                        ) : (
                  ...
                  <div className="mt-auto">
                    <p className="text-md font-semibold text-black mb-4">
                      {foodPlace.lowest_price === null &&
                      foodPlace.highest_price === null ? (
                        ...
                        <>
                          ...
                          <span className="text-red-500 text-xl font-bold ml-2">
                            ₱
                            {foodPlace.lowest_price -
                              (foodPlace.highest_price * foodPlace.discount) / 100}
                          </span>
                        </>
                      ) : (
                        <>
