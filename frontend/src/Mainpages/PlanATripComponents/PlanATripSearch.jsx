...
const PlanATripSearch = () => {
  ...
  return (
    <div className="flex mt-1 flex-col items-center p-5 w-full max-w-2xl mx-auto">
      ...
      {searchResults.length > 0 && (
        <motion.div
          className="relative w-full z-50 max-h-60 overflow-y-auto bg-white shadow-lg rounded-lg"
        ...
        >
          {searchResults.map((result, index) => (
            <Link
              key={index}
              to={result.businessInfo ? `/business/${encryptId(result.businessInfo.id)}` : `/destinations?name=${result.value}`}
...
export default PlanATripSearch;
